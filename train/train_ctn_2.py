...
#*********************************************
from config.config_ctn_2 import Config
from public.detection.models.utils.utils import _tranpose_and_gather_feature, load_model
from public.detection.models.ctn_loss import _neg_loss, _reg_loss
...
#     images, annotations = prefetcher.next()
    ...
    for batch_idx, batch in enumerate(train_loader):
        for k in batch:
            if k != 'meta':
                batch[k] = batch[k].to(device=torch.device('cuda:%d' % local_rank), non_blocking=True)

        heatmap_output, offset_output, wh_output = model(batch['image'])
        outputs = [[heatmap_output, offset_output, wh_output]]
        hmap, regs, w_h_ = zip(*outputs)
        regs = [_tranpose_and_gather_feature(r, batch['inds']) for r in regs]
        w_h_ = [_tranpose_and_gather_feature(r, batch['inds']) for r in w_h_]

        heatmap_loss = _neg_loss(hmap, batch['hmap'])
        offset_loss = _reg_loss(regs, batch['regs'], batch['ind_masks'])
        wh_loss = _reg_loss(w_h_, batch['w_h_'], batch['ind_masks'])
        loss = heatmap_loss + 1 * offset_loss + 0.1 * wh_loss
...
#         torch.nn.utils.clip_grad_norm_(model.parameters(), 0.1)
        ...
        if local_rank == 0 and batch_idx % args.print_interval == 0:
            logger.info(
                f"train: epoch {epoch:0>3d}, iter [{batch_idx:0>5d}, {iters:0>5d}], heatmap_loss: {heatmap_loss.item():.2f}, offset_loss: {offset_loss.item():.2f}, wh_loss: {wh_loss.item():.2f}, loss_total: {loss.item():.2f}"
            )
